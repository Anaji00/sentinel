...
from shared.utils import quant_calc
    ...
    def calculate_garch_volatility(self, returns: List[float], omega: float = 0.000002, alpha: float = 0.08, beta: float = 0.90) -> float:
        """Calculates GARCH(1,1) conditional volatility forecast via quant_calc."""
        if not returns or len(returns) < 5:
            return 0.20
        return quant_calc.garch_volatility(returns, omega=omega, alpha=alpha, beta=beta, annualize=True)

    def calculate_kelly_position_size(self, win_rate: float, win_loss_ratio: float, kelly_fraction: float = 0.5) -> float:
        """Computes fractional Kelly Criterion position sizing via quant_calc."""
        return quant_calc.kelly_criterion(win_rate, win_loss_ratio, half_kelly=(kelly_fraction <= 0.5))
    ...
    async def handle(self, message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # ── STEP 5: Write Episodic Memory & Structured Bulletin ────────────────
        if added or anomaly_score >= 0.8:
            ...
            # Structured Agent Bulletin publication for cross-agent synthesis
            asyncio.create_task(self.publish_bulletin(
                bulletin_type="thesis",
                summary=f"Quant discovery on {ticker}: {discovery.catalyst_category}. Peers: {peer_list[:4]}",
                ticker=ticker,
                conviction=min(1.0, anomaly_score * 0.9),
                expected_direction="uncertain",
                payload={"peers": peer_list, "catalyst": discovery.catalyst_category, "notional_usd": notional},
                ttl_seconds=3600,
            ))
