...
class FinancialAdvisorAgent(SentinelAgent):
    ...
    async def handle(self, message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        bulletins_context = await self.get_bulletins_for_prompt()
        ...
        try:
            ...
            # Write advice summary to shared memories & record predictions/bulletins
            for play in response.highest_conviction_plays[:3]:
                ...
                # Track prediction for self-calibration
                ...
                # Publish structured bulletin for consensus engine
                asyncio.create_task(self.publish_bulletin(
                    bulletin_type="signal",
                    summary=f"{play.action} {play.ticker} @ ${play.entry_level:.2f} -> ${play.target_price:.2f} (R:R {play.risk_reward_ratio:.1f})",
                    ticker=play.ticker,
                    conviction=play.conviction_score,
                    expected_direction=direction,
                    payload={"action": play.action, "entry": play.entry_level, "target": play.target_price, "kelly_pct": play.kelly_allocation_pct},
                    ttl_seconds=3600,
                ))
